...
1. **立即进入方案设计模式**，禁止直接编码
2. 理解需求并确认
3. **平台/版本兼容性检查（强制）**：项目最低支持 **JDK 17 / Spring Boot 2.7.3 / Node 20**，所有 API 必须兼容
...
**工作流程**：
1. 确认项目最低支持版本（当前：JDK 17 · Spring Boot 2.7.3 · Node 20 / Vite 6）
2. 检查 API 兼容性
3. 如使用新版本 API，提供降级方案
...
1. **编译验证**：
   - 后端：`mvn -q -B clean package -DskipTests`（产出 `target/iyque-code-1.0-SNAPSHOT.jar`）
   - PC 前端：`cd frontEnd/pc && npm run build`
   - 移动端：`cd frontEnd/mobile && npm run build`
   - 只改了单侧就只构建那一侧，不必全量。
2. **运行策略**：**允许在本机直接运行验证**。用 `./deploy/start-local.sh` 起全套
   （Docker 里的 MySQL+Redis、后端 8085、PC 2024、移动端 1026），
   `./deploy/stop-local.sh` 停止。验证完接口可直接 curl。
   ⚠️ **本机运行 ≠ 生产部署**。往生产服务器推（`deploy/server/deploy.sh`）属于对外动作，必须先经用户授权。
